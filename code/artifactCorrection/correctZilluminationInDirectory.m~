function varargout=correctZilluminationInDirectory(sectionDirectory,outputDirectory,verbose,method)
...
% modified Natalia Chicherova 2017
...
parfor (ii=1:nPhys,nWorkers)
    ...
    %Correct the data
    switch method
        case 'smoothingRescale'
            thisSection=smoothingRescale(thisSection,verbose);
        case 'MeanInten'
            thisSection=MeanIntensityCorrection(thisSection,verbose);
    end
...
function thisSection=MeanIntensityCorrection(thisSection,verbose)
% Algorithm:
% 0. Calculate Image mean intensity for the first section
% 1. Multiply the other images by a number from 1 to 3
% 2. Find a minima of mean intensity differences depending on multiplication
% factor
% 3. Correct the other images by the factor

    if verbose
        disp('Correcting brightness using mean Intensities...')
    end
    
    thisSection=single(thisSection);
    RefImage = thisSection(:,:,1);
    % limit brightness difference to 3 times max
    ra = linspace(1,3,20);
    RA = zeros(size(ra));
    %Now go through and divide
    for jj=2:size(thisSection,3)
        nextSection = thisSection(:,:,jj);
        for i =1:20
            BrighterNextSection = nextSection.*ra(i);
            RA(i) = abs(mean(RefImage(:))-mean(BrighterNextSection(:)));
        end
        %Now correct each section by this factor
        thisSection(:,:,jj)=thisSection(:,:,jj).*min(RA);
    end
